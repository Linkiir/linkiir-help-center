...
| --- | --- |
| **Grid** | The browser UI and API at `http://127.0.0.1:8080`. Projects, workflows, nodes, scripts, users, settings, and log search. |
...
The Grid supervises the Runtime and Log Archiver for you. If the Grid is running and healthy, they are too — check `http://127.0.0.1:8080/api/health` to confirm.

## Where things live

| Platform | Your projects and settings | Grid |
| --- | --- | --- |
...
Every message carries a **message ID** and a **correlation ID** through the whole workflow. The correlation ID is the same at every node, so searching it in the Grid's log search returns the complete journey of one message — received, transformed, delivered.
